...
class CZIReader(object):
    def __init__(self, czi_path):
        ...
        if self.is_rgb:
            ...
            single_channel_tags = [metadata[ds_ind][ch_ind][0][i].tag for i in range(len(metadata[ds_ind][ch_ind][0]))]
            channel_vals = []
            for i in range(len(single_channel_tags)):
                channel_vals.append([metadata[ds_ind][ch_ind][0][i].text, metadata[ds_ind][ch_ind][1][i].text])
            hex_color = channel_vals[np.where(np.ravel(single_channel_tags) == 'Color')[0][0]]
            self.channel_name = channel_vals[np.where(np.ravel(single_channel_tags) == 'ShortName')[0][0]]
            self.rgb_colors = []
            self.hsv_colors = []
            for i in range(len(ch_tags)):
                da_color = hex_color[i][0] + hex_color[i][3:]
