...
def get_probe_length(segmentation_data, sp, ep, avg, direction, resolution=39):
    um_total_length = np.sqrt(np.sum((sp - ep)**2)) * resolution - 175
    pix_total_length = um_total_length / resolution
    # ts = np.sqrt(np.sum((sp - avg)**2))
    # te = pix_total_length - ts
    total_chn2 = int(np.round(um_total_length / 20))
    chk_vox = np.zeros((total_chn2,3))
    step_length = pix_total_length / total_chn2
    
    for i in range(total_chn2):
        chk_vox[i] = sp + step_length * i * direction
    
    chk_vox = chk_vox + 0.5 * (chk_vox[1]-chk_vox[0])
    chk_vox = np.floor(chk_vox).astype(int)
    
    unique_label = []
    region_labels = np.zeros(total_chn2)
    for i in range(total_chn2):
        region_labels[i] = segmentation_data[chk_vox[i,0], chk_vox[i,1], chk_vox[i,2]]
        if i == 0:
            unique_label.append(region_labels[i])
        else:
            if region_labels[i] not in unique_label:
                unique_label.append(region_labels[i])

    unique_label = np.ravel(unique_label).astype(int)
    region_length = np.zeros(len(unique_label))
    channels = np.zeros(len(unique_label))
    for i in range(len(unique_label)):
        channels[i] = np.sum(region_labels == unique_label[i]) * 2
        region_length[i] = np.sum(region_labels == unique_label[i]) * 20
        
    return unique_label, region_length, channels, chk_vox[-1]


def get_label_name(label_info, unique_label):
    label_names = []
    label_acronym = []
    label_color = []
    for i in range(len(unique_label)):
...
def apply_affine_transform(src, src_tri_pnts, dst_tri_pnts, size):
    ...
    dst = cv2.warpAffine(src.astype(np.float32), warp_mat, (size[0], size[1]), None, flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT_101)
    return dst
...
def warp_triangle(img1, img2, t1, t2, is_rgb=False):
    ...
    if is_rgb:
        ...
        mask = np.zeros((r2[2], r2[3]), dtype=np.float32)
        cv2.fillConvexPoly(mask, np.int32(t2_rect_int[:, ::-1]), 1, 16, 0)

    print(mask.shape)
    # Apply warpImage to small rectangular patches
    img1_rect = img1[r1[1]:r1[1] + r1[3], r1[0]:r1[0] + r1[2]]
    # img2Rect = np.zeros((r2[3], r2[2]), dtype = img1Rect.dtype)
    print('img1_rec', img1_rect.shape)

    size = (r2[2], r2[3])

    img2_rect = apply_affine_transform(img1_rect, t1_rect, t2_rect, size)
    print('img2_rec', img2_rect.shape)
...
# def make_contour_img(ct, size):
#     contour_img = np.zeros(size, 'uint8')
#     if len(ct) > 0:
...
def render_volume(atlas_data, atlas_folder, factor=2, level=0.1):
    ...
    verts, faces = pg.isosurface(ndi.gaussian_filter(img.astype('float32'), (2, 2, 2)), np.max(atlas_data) * level)
